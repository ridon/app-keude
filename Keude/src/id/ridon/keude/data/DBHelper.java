...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String TAG = "id.ridon.keude.data.DBHelper";
    ...
    private void populateRepoNames(SQLiteDatabase db, int oldVersion) {
        if (oldVersion < 37) {
            Log.i("Keude", "Populating repo names from the url");
            ...
            if (cursor != null) {
                if (cursor.getCount() > 0) {
                    ...
                    while (!cursor.isAfterLast()) {
                        ...
                        Log.i("Keude", "Setting repo name to '" + name + "' for repo " + address);
    ...
    private void renameRepoId(SQLiteDatabase db, int oldVersion) {
        if (oldVersion < 36 && !columnExists(db, TABLE_REPO, "_id")) {
            ...
            Log.d("Keude", "Renaming " + TABLE_REPO + ".id to _id");
            db.beginTransaction();
            ...
            try {
                ...
                Log.e("Keude", "Error renaming id to _id: " + e.getMessage());
            }
            db.endTransaction();
        }
    }
    ...
    private void insertRepo(
        ...
        Log.i("Keude", "Adding repository " + name);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        Log.i("Keude", "Upgrading database from v" + oldVersion + " v"
                + newVersion);
    ...
    private void addLastUpdatedToRepo(SQLiteDatabase db, int oldVersion) {
        if (oldVersion < 35 && !columnExists(db, TABLE_REPO, "lastUpdated")) {
            Log.i("Keude", "Adding lastUpdated column to " + TABLE_REPO);
    ...
    private void resetTransient(SQLiteDatabase db, int oldVersion) {
        ...
        if (oldVersion < 42) {
            context.getSharedPreferences("Keude", Context.MODE_PRIVATE).edit()
