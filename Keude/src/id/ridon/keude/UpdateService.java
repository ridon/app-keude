...
import id.ridon.keude.updater.RepoUpdater;
...
public class UpdateService extends IntentService implements ProgressListener {
    ...
    private boolean verifyIsTimeForScheduledRun() {
        ...
        if (elapsed < interval * 60 * 60 * 1000) {
            Log.d("Keude", "Skipping update - done " + elapsed
    ...
    private void showAppUpdatesNotification(int updates) {
        ...
        Intent notifyIntent = new Intent(this, Keude.class)
                .putExtra(Keude.EXTRA_TAB_UPDATE, true);
        if (updates > 1) {
        ...
        TaskStackBuilder stackBuilder = TaskStackBuilder
                .create(this).addParentStack(Keude.class)
     ...
     * {@link id.ridon.keude.UpdateService#getKnownApks(java.util.List)}
     * instead, which will only call this with the right number of apks at
     * a time.
     * @see id.ridon.keude.UpdateService#getKnownAppIds(java.util.List)
     */
    ...
    private void updateOrInsertApks(List<Apk> apksToUpdate, int totalApksAppsCount, int currentCount) {
        ...
        Log.d("Keude", "Updating/inserting " + operations.size() + " apks.");
        try {
    ...
    private void removeApksNoLongerInRepo(List<Apk> apksToUpdate, List<Repo> updatedRepos) {
        ...
        Log.d("Keude", "Found " + toRemove.size() + " apks no longer in the updated repos (took " + duration + "ms)");
    ...
    private void removeApksFromRepos(List<Repo> repos) {
        for (final Repo repo : repos) {
            ...
            Log.d("Keude", "Removing " + numDeleted + " apks from repo " + repo.address);
        }
    }

    private void removeAppsWithoutApks() {
        int numDeleted = getContentResolver().delete(AppProvider.getNoApksUri(), null, null);
        Log.d("Keude", "Removing " + numDeleted + " apks that don't have any apks");
    }
